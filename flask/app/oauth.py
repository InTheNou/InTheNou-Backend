...
blueprint = make_google_blueprint(
    ...
    scope=["profile", "email"],
    storage=SQLAlchemyStorage(OAuth, db.session,user_required=True, user=current_user),
    offline=True
)
...
def google_logged_in(blueprint, token):
   ...
   # print (token)
    ...
    query = OAuth.query.filter_by(token= str(token['access_token']),user=user,id=user.id)
    try:
        oauth = query.one()
    #user was not found in the database
    except NoResultFound:
        ...
        if(user):
            oauth = OAuth(provider=blueprint.name,token=str(token['access_token']),created_at=(str(token['expires_at'])),id=user.id,user=user)
    ...
    cookie = encodeFlaskCookie(secret_key=os.getenv("FLASK_SECRET_KEY"), cookieDict=sessionDict)
    #print(cookie)
